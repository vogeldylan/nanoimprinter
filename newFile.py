...
class dataLog:
    ...
    def createPlot(self, window):
        ...
        # plot data
        window.graphWidget.canvas.ax.plot(window.process.times, window.process.cent_temps, 'r', window.process.times, window.process.edge_temps, 'b')
        #window.graphWidget.canvas.ax.plot(window.process.times, window.process.edge_temps, 'b')
        window.graphWidget.canvas.ax.set_xlabel('Time From Start (s)')
        window.graphWidget.canvas.ax.set_ylabel('Temperature (°C)')
        window.graphWidget.canvas.ax.set_title('Temperature Measurements Over Time '+ str(window.val))
    ...
    def createPlotTemp(self, window):
        #window.graphWidget.figure.clear()
        #window.graphWidget.canvas.clear()
    ...
    def updatePlot(self, window):
        ...
        if(window.process.mode == "Heating mode"):
            window.graphWidget.canvas.ax.plot(window.process.times, window.process.cent_temps, 'r', label= 'Center Temp')
            window.graphWidget.canvas.ax.plot(window.process.times, window.process.edge_temps, 'b', label= 'Edge Temp')
            window.graphWidget.canvas.ax.plot(window.process.times, target_temps, 'g-', label = 'Target Temp')
        else:
            window.graphWidget.canvas.ax.plot(window.process.times, window.process.cent_temps, 'r', window.process.times, window.process.edge_temps, 'b')
        #window.graphWidget.canvas.ax.plot(window.process.times, window.process.edge_temps, 'b')
        window.graphWidget.canvas.ax.set_xlabel('Time From Start (s)')
        ...
        window.graphWidget.canvas.ax.legend(loc='lower left', shadow = True)
        # refresh canvas
        now = datetime.datetime.now()
    ...
    def write_line_to_log(self, window): #change
        ...
        window.displayMessage('[Temp_1: ' + str(round(window.process.t_center, 3)) + ']   ' + '[Duty_center: ' + str(round(window.process.pwm_center, 3)) + ']')
        #window.displayMessage('[Duty_center: ' + str(round(window.process.pwm_center, 3)) + ']')
        window.outputMessage.setTextColor(QtGui.QColor(0,0,255))
        window.displayMessage('[Temp_2: ' + str(round(window.process.t_edge, 3)) + ']   ' + '[Duty_edge: ' + str(round(window.process.pwm_edge, 3)) + "]")
        #window.displayMessage('[Duty_edge: ' + str(round(window.process.pwm_edge, 3)) + "]")
    ...
    def write_temp_to_log(self, window):
        ...
        #window.outputMessage.setTextBackgroundColor(QtGui.QColor(255,255,0))
        #window.displayMessage('[Time: ' + str(d_time) + ']\n' + '[Temp_1: ' + str(window.process.t_center) + ']\t' + '[Temp_2: ' + str(window.process.t_edge) + ']\t')
        window.displayMessage('[Time: ' + str(d_time) + ']')
        window.outputMessage.setTextColor(QtGui.QColor(255,0,0))
